...
global SRCROOT ../src/hospcompare
...
foreach release in 201304 {
	di "****** RELEASE `release'**********"
	
	local ext csv
	
	insheet hosp_id hosp_name state condition measure measure_label score sample_size footnote using $SRCROOT/dbo_vwHQI_HOSP_MSR_XWLK_d`release'.`ext', clear nonames
...
*Write a 'xwalk' for the early measure labels. Note that outpatient "OP-" measures don't exist in early data and "thrombolytics", "oxygenation", "beta blockers at arrival" and "left ventricular assessment" don't exist in late data. Therefore, these are discarded for now (July 15, 2013)
...
foreach var in $MSRLIST {
	di "**** variable `var'*****"
	ren score`var' `var'
	ren sample`var' `var'_N
}

drop score* sample* hospyr

save ../dta/temp-coremsr, replace
